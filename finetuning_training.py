...
ei_hat = ei_hat['ei'].unsqueeze(0).to(device)

G = Generator(frame_shape)
D = Discriminator(batch_size, len(dataset), path_to_Wi, finetuning=True)
...
checkpoint['D_state_dict']['W_i'] = torch.randn(batch_size,768,1)#making sure placeholders are correct shaped
checkpoint['D_state_dict']['w_prime'] = torch.randn(batch_size,768,1)#making sure placeholders are correct shaped
...
for epoch in range(num_epochs):
    for i_batch, (pose_aug, pose_img, pose_seg) in enumerate(dataLoader):
        ...
        with torch.autograd.enable_grad():
            ...
            r_hat, D_hat_res_list = D(x_hat, e_finetuning=e_hat)
            r, D_res_list = D(x, e_finetuning=e_hat)
